...
// BuildImage builds the docker image based on the parameters obtained from the yaml file
func BuildImage(buildContextDir, tagName string) {
	...
	imageBuildResponse, err := cli.ImageBuild(ctx, tarReader, buildOpts)
...
// ScanDir function scans the build context directory for writing to the tarball
...
// TarWrite writes the contents of the build context directory to the tarball header
